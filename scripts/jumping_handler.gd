...
export var jump_height = 64 * 4 setget set_jump_height, get_jump_height
...
func process_state(delta):
	...
	if(not Input.is_action_pressed("jump") or get_timer() > get_parent().calculate_max_airtime(get_jump_height(), get_parent().get_gravity_vector().y)):
		...
		var collision_info = get_parent().move_and_collide(Vector2(1,0) * get_parent().get_direction() * pressed * delta * get_parent().get_movement_speed())
