...
func process_state(delta):
	...
	if(pushed_timer < damage_push_time):
		if(Input.is_action_just_pressed("jump") and pushed_timer > time_to_control):
			get_parent().set_velocity(Vector2(0,0))
			get_parent().set_state("JUMPING")
		
		elif((not Input.is_action_pressed("play_right") and not Input.is_action_pressed("play_left")) or pushed_timer <= time_to_control):
